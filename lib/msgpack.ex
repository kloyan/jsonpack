defmodule MsgPack do
    ...
    defp pack_list(x) when length(x) < @bit_4, do: << 0b1001::4, length(x)::4, pack_list(x, <<>>)::binary >>
    defp pack_list(x) when length(x) < @bit_16, do: << 0xDC, length(x)::16, pack_list(x, <<>>)::binary >>
    defp pack_list(x) when length(x) < @bit_32, do: << 0xDD, length(x)::32, pack_list(x, <<>>)::binary >>
